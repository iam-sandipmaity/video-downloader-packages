...
echo "=== Packaging Custom Python APK ==="

# Check if target binaries exist
ABI="arm64-v8a"
BINARY_SO="app/src/main/jniLibs/$ABI/libpython.so"
BINARY_ZIP="app/src/main/jniLibs/$ABI/libpython.zip.so"

if [ ! -f "$BINARY_SO" ] || [ ! -f "$BINARY_ZIP" ]; then
    echo "Error: libpython.so or libpython.zip.so not found under app/src/main/jniLibs/$ABI/"
    echo "Please compile Python using ./build_python.sh first."
    exit 1
fi
